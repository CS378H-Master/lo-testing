...
- `LO-4/InvalidPrograms/` — compile-error tests: field shadowing (`E_FIELD_SHADOWING`), extends-non-class (`E_EXTENDS_NON_CLASS`), both `super(...)` and `this(...)` in one constructor (`E_DELEGATION_BOTH_SUPER_AND_THIS`), reserved variable name (`E_RESERVED_VARIABLE_NAME`), and cast-target-not-class (`E_CAST_TARGET_NOT_CLASS`). Under the WS-6 follow-up the runtime-abort and valid tests that had been misfiled here were moved out and the directory renumbered to unique sequential `test_<N>`.
