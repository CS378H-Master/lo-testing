...
- `LO-4/ValidPrograms/` — new tests covering inheritance basics and `super`/`this` dispatch, constructor chains, polymorphic vtable dispatch, casts, `instanceof`, same-name methods across unrelated classes, null-field defaults, and basic I/O (`println`, read/write round-trip, read-until-EOF). Written directly in redesigned form; not part of the migration. Several valid tests that had been misfiled in `InvalidPrograms/` were re-sorted here under the WS-6 follow-up, and the directory was renumbered to unique sequential `test_<N>`.
- `LO-4/InvalidPrograms/` — compile-error tests: field shadowing (`E_FIELD_SHADOWING`), extends a non-class type keyword (`E_RESERVED_KEYWORD_AS_IDENTIFIER`, a parse error), both `super(...)` and `this(...)` in one constructor (`E_DELEGATION_BOTH_SUPER_AND_THIS`), reserved variable name (`E_RESERVED_VARIABLE_NAME`), and cast-target-not-class (`E_CAST_TARGET_NOT_CLASS`). Under the WS-6 follow-up the runtime-abort and valid tests that had been misfiled here were moved out and the directory renumbered to unique sequential `test_<N>`.
