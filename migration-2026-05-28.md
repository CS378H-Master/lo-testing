...
Two of the migrated examples contain pre-existing bugs demonstrated by the test; both have now been annotated with `expected compile error:` lines, and the existing vocabulary in `error-codes.md` already covered both (no new codes were needed):

- `LO-3/InvalidPrograms/test_4.lo` — the program contains `a = Tester(5);` (a call written without `new`) and `int getA{` (a method header missing its formal-parameter parentheses). Both are **parse-phase** failures, not a semantic error: there is deliberately no `E_MISSING_NEW`, because a missing `new` doesn't parse as anything — `<Var> ( <Actuals> )` is not an LO-3 expression production. Annotated `E_PARSE_PHASE_OTHER` (the parse-phase sentinel; the harness substring-matches, so a compiler emitting a more specific diagnostic still passes). The misleading original "Missing new in constructor" description was corrected. (Hygiene note: the file demonstrates two distinct malformations; a future pass could split it into two focused tests, but both are the same category so the single annotation is correct as-is.)
- `LO-3/InvalidPrograms/test_8.lo` — declares `class A() {...}` twice. Annotated `E_DUPLICATE_CLASS_NAME`, which **already exists** in `error-codes.md` under the well-formedness phase. (An earlier draft of this note claimed that code was missing; that was inaccurate — it is enumerated.)

The remaining pre-redesign `InvalidPrograms` tests are annotated as the systematic retrofit proceeds, *after* the brace migration (B) lands them in current-grammar form. The brace migration must preserve all header comments verbatim and must not touch `expected compile error:` lines (the migration and the error-code retrofit are separate passes; see `runbooks/ws4-corpus-migration.md`).
